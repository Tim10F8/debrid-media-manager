...
class TrackerManager {
	private trackers: string[] | null = null;
	...
	async getTrackers(): Promise<string[]> {
		...
		if (this.trackers && now - this.lastFetch < CACHE_DURATION) {
			return this.trackers;
		}

		// Fetch new tracker list
		for (const url of TRACKER_URLS) {
			try {
				const response = await axios.get(url, {
					timeout: 10000,
					headers: {
						'User-Agent': 'DMM-Tracker-Fetcher/1.0',
					},
				});

				// Parse the tracker list (newline separated, filter empty lines)
				...
				if (trackers.length > 0) {
					...
					this.lastFetch = now;
					console.log(`Successfully fetched ${trackers.length} trackers from ${url}`);
					return trackers;
				}
			} catch (error) {
				console.error(`Failed to fetch trackers from ${url}:`, error);
				// Continue to next URL
			}
		}

		// If all URLs fail, return previously cached trackers if available
		if (this.trackers) {
			console.warn('Using stale tracker cache as all fetch attempts failed');
			return this.trackers;
		}

		// Last resort: return a minimal set of reliable trackers
		console.error('All tracker fetches failed, using fallback trackers');
		return [
			...
			'http://45.146.254.59:80/announce',
			...
			'http://34.94.76.146:80/announce.php',
			...
			'http://34.89.91.10:80/announce.php',
	...
	// Force refresh the tracker cache
	async refreshTrackers(): Promise<string[]> {
		this.trackers = null;
		this.lastFetch = 0;
